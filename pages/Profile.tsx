...
import { BloomMeter } from '../components/BloomMeter';
import { AnimatedBgSelector } from '../components/AnimatedBackground';
...
export const Profile: React.FC = () => {
  const { currentUser, updateUserProfile, logoutUser, streamingQuality, setStreamingQuality, favoriteArtists, themeColor, setThemeColor, totalListeningSeconds, weeklyListeningSeconds, playCounts, history, listeningStreak, miniPlayerStyle, setMiniPlayerStyle, crossfadeSeconds, setCrossfade, dataSaverMode, setDataSaver, userBio, setUserBio, userBanner, setUserBanner } = usePlayerStore();
  const navigate = useNavigate();

  const personalityBadge = React.useMemo(() => {
    const totalH = totalListeningSeconds / 3600;
    const hour = new Date().getHours();
    const counts = Object.values(playCounts);
    const avgPlays = counts.length ? counts.reduce((a,b) => a+b, 0) / counts.length : 0;
    if (hour >= 22 || hour < 4) return { label: 'Night Owl 🦉', desc: 'Raat ko sabse zyada sunna pasand hai', color: '#6366F1' };
    if (totalH > 50) return { label: 'Music Addict 🎧', desc: '50+ ghante — real fan!', color: '#FF6B9D' };
    if (avgPlays > 5)  return { label: 'Repeat Lover 🔁', desc: 'Ek song baar baar sunna', color: '#A855F7' };
    if (listeningStreak > 7) return { label: 'Streaker 🔥', desc: listeningStreak + ' din ki streak!', color: '#F59E0B' };
    return { label: 'Music Explorer 🌸', desc: 'Naye songs dhundna pasand hai', color: '#10B981' };
  }, [totalListeningSeconds, playCounts, listeningStreak]);
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      setBio(userBio || '');
      setBannerPreview(userBanner || null);
    } else {
      navigate('/login');
    }
      ...
      >
          ...
          <form onSubmit={handleSave} className="flex flex-col gap-6 mt-8">
              ...
              >
               ...
               {/* Banner */}
               <motion.div variants={itemVariants} className="rounded-2xl overflow-hidden border border-[#FF6B9D]/10 mb-3">
                 <div className="relative h-24 bg-gradient-to-r from-[#1E0030] to-[#2A0038] cursor-pointer"
                   onClick={() => bannerInputRef.current?.click()}
                   style={bannerPreview ? { backgroundImage: `url(${bannerPreview})`, backgroundSize: 'cover', backgroundPosition: 'center' } : {}}>
                   {!bannerPreview && <div className="absolute inset-0 flex items-center justify-center text-[#D4A0BA]/40 text-sm">Tap to add cover photo 📸</div>}
                   <div className="absolute bottom-2 right-2 bg-black/50 rounded-full px-2 py-1 text-[10px] text-white/60">Change</div>
                 </div>
               ...
               {/* Bio */}
               <motion.div variants={itemVariants} className="bg-[#130018] rounded-2xl p-4 border border-[#FF6B9D]/10 mb-3">
                 <p className="text-white/60 text-xs font-bold mb-2">📝 Bio</p>
                 <textarea
                   value={bio} onChange={e => setBio(e.target.value)} maxLength={150}
                   placeholder="Apne baare mein kuch likho... 🌸"
                   rows={3}
                   className="w-full bg-[#1A0025] border border-[#FF6B9D]/20 rounded-xl px-3 py-2 text-white text-sm resize-none focus:outline-none focus:border-[#FF6B9D]/50 placeholder-[#D4A0BA]/30"
                 />
                 <p className="text-white/20 text-[10px] text-right mt-1">{bio.length}/150</p>
               </motion.div>

               {/* Animated Background */}
               <motion.div variants={itemVariants} className="bg-[#130018] rounded-2xl p-4 border border-[#FF6B9D]/10 mb-3 flex items-center justify-between">
                 <div>
                   <p className="text-white font-bold text-sm">Animated Background</p>
                   <p className="text-[#D4A0BA] text-xs">App ka background customize karo</p>
                 </div>
               ...
               <motion.div variants={itemVariants} className="bg-[#130018] rounded-2xl p-4 border border-[#FF6B9D]/10 mb-3 flex items-center justify-between">
                 <div>
                   <p className="text-white font-bold text-sm">📶 Data Saver Mode</p>
                   <p className="text-[#D4A0BA] text-xs">Mobile data pe low quality</p>
                 </div>
                 <button onClick={() => setDataSaver(!dataSaverMode)}
                   className={`w-12 h-6 rounded-full transition-all relative ${dataSaverMode ? 'bg-[#FF6B9D]' : 'bg-[#2A0038]'}`}>
                   <div className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-all ${dataSaverMode ? 'left-6' : 'left-0.5'}`}/>
                 </button>
               </motion.div>

               {/* Wrapped shortcut */}
               <motion.div variants={itemVariants} className="mb-3">
                 <motion.button whileTap={{ scale: 0.97 }} onClick={() => navigate('/wrapped')}
                   className="w-full py-3 rounded-2xl font-bold text-white flex items-center justify-center gap-2 border border-[#FF6B9D]/30"
                   style={{ background: 'linear-gradient(135deg,rgba(255,107,157,0.15),rgba(194,24,91,0.08))' }}>
                   🎵 Monthly Wrapped Card Dekho
                 </motion.button>
               </motion.div>

               {/* Personality Badge */}
               <motion.div variants={itemVariants} className="rounded-2xl p-4 border mb-3 flex items-center gap-4"
                 style={{ background: `linear-gradient(135deg, ${personalityBadge.color}20, ${personalityBadge.color}08)`, borderColor: `${personalityBadge.color}30` }}>
                 <div className="text-4xl">🏅</div>
                 <div>
                   <p className="text-white font-black text-base">{personalityBadge.label}</p>
                   <p className="text-white/50 text-xs">{personalityBadge.desc}</p>
                 </div>
               </motion.div>

               {/* Mini Player Style */}
               <motion.div variants={itemVariants} className="bg-[#130018] rounded-2xl p-5 border border-[#FF6B9D]/10 mb-3">
                 <p className="text-white font-bold text-sm mb-3">Mini Player Style</p>
                 <div className="grid grid-cols-3 gap-2">
                   {(['bar','island','bubble'] as const).map(style => (
                     <button key={style} onClick={() => setMiniPlayerStyle(style)}
                       className={`py-2.5 rounded-xl text-xs font-bold border transition-all capitalize ${
                         miniPlayerStyle === style ? 'border-[#FF6B9D]/60 bg-[#FF6B9D]/15 text-[#FF6B9D]' : 'border-white/10 text-white/50 hover:border-white/20'
                       }`}>
                       {style === 'bar' ? '▬ Bar' : style === 'island' ? '⬤ Island' : '● Bubble'}
                     </button>
                   ))}
                 </div>
               </motion.div>
               ...
               <motion.div variants={itemVariants} className="bg-[#130018] rounded-2xl p-5 border border-[#FF6B9D]/10 mb-3">
                 <div className="flex items-center justify-between mb-3">
                   <p className="text-white font-bold text-sm">Crossfade</p>
                   <span className="text-[#FF6B9D] font-bold text-sm">{crossfadeSeconds}s</span>
                 </div>
                 <input type="range" min={0} max={12} step={1} value={crossfadeSeconds}
                   onChange={e => setCrossfade(Number(e.target.value))}
                   className="w-full h-2 rounded-full appearance-none cursor-pointer"
                   style={{ accentColor: '#FF6B9D', background: `linear-gradient(to right, #FF6B9D ${crossfadeSeconds/12*100}%, #2A0038 ${crossfadeSeconds/12*100}%)` }}
                 />
