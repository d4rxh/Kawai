...
export const DownloadQualityModal: React.FC<DownloadQualityModalProps> = ({ song, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-[#0A000F]/90  p-4 animate-in fade-in duration-200">
        <motion.div 
            initial={{ scale: 0.9, opacity: 0, y: 20 }}
            animate={{ scale: 1, opacity: 1, y: 0 }}
            exit={{ scale: 0.9, opacity: 0, y: 20 }}
            className="bg-[#130018] w-full max-w-2xl rounded-xl overflow-hidden relative"
        >
            {/* Header */}
            <div className="p-8 pb-4 flex items-start justify-between">
                ...
                <button 
                    onClick={onClose} 
                    className="p-3 hover:bg-[#2A0038] rounded-full text-white transition-colors"
                >
                    <X size={24} />
                </button>
            </div>
                        ...
                        {sortedQualities.map((option, idx) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="relative bg-[#2A0038] h-full p-5 rounded-lg flex items-center justify-between">
                                        ...
                                        <div className="w-10 h-10 rounded-full bg-[#3D0050] flex items-center justify-center group-hover:bg-accent group-hover:text-black transition-all">
                                            <Download size={20} />
                                        </div>
                                    </div>
                                </motion.button>
            ...
            {/* Footer */}
            <div className="bg-[#130018] p-4 text-center text-xs text-white/30 uppercase tracking-widest font-bold">
