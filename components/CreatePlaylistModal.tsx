...
export const CreatePlaylistModal: React.FC<CreatePlaylistModalProps> = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[250] flex items-end md:items-center justify-center p-0 md:p-4 bg-[#0A000F] ">
      <motion.div 
        initial={{ y: 50, opacity: 0, scale: 0.95 }}
        animate={{ y: 0, opacity: 1, scale: 1 }}
        exit={{ y: 50, opacity: 0, scale: 0.95 }}
        transition={{ type: "tween", duration: 0.25, ease: "easeOut" }}
        className="bg-[#130018] w-full max-w-lg rounded-t-xl md:rounded-xl overflow-hidden flex flex-col max-h-[95dvh]"
      >
        
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-white/5 bg-[#130018]">
            <h2 className="text-xl font-bold text-white flex items-center gap-2">
                Create Playlist
                {selectedSeed && <Sparkles size={16} className="text-accent" />}
            </h2>
            <button onClick={onClose} className="p-2 hover:bg-[#2A0038] rounded-full text-white/70 hover:text-white transition-colors">
                <X size={20} />
            </button>
        </div>
        ...
        <div className="overflow-y-auto no-scrollbar">
            <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-6">
                ...
                <div className="flex flex-col md:flex-row gap-6">
                    ...
                    <div className="flex justify-center shrink-0">
                        <div 
                            ...
                            className="w-40 h-40 bg-[#2A0038] rounded-lg flex flex-col items-center justify-center cursor-pointer hover:bg-[#3D0050] transition-colors group relative overflow-hidden"
                        >
                            {preview ? (
                    ...
                    <div className="space-y-4 flex-1">
                        <div className="flex flex-col gap-1.5">
                            ...
                            <input 
                                ...
                                className="bg-[#2A0038] text-white p-3 rounded-md focus:outline-none focus:ring-1 focus:ring-white/30 font-medium w-full"
                        ...
                        <div className="flex flex-col gap-1.5">
                            ...
                            <textarea 
                                ...
                                className="bg-[#2A0038] text-white p-3 rounded-md focus:outline-none focus:ring-1 focus:ring-white/30 resize-none h-20 text-sm w-full"
                            />
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-[#1E0028] p-4 rounded-lg relative">
                    ...
                    {!selectedSeed ? (
                        <div className="relative">
                            ...
                            <input 
                                ...
                                className="w-full bg-[#2A0038] text-white pl-9 pr-4 py-2.5 rounded-lg text-sm focus:outline-none transition-colors"
                            />
                            {isSearchingSeed && <Loader2 size={16} className="animate-spin absolute right-3 top-1/2 -translate-y-1/2 text-white/50" />}
                            
                            {/* Dropdown Results */}
                            {seedResults.length > 0 && (
                                <div className="absolute top-full left-0 right-0 mt-2 bg-[#2A0038] rounded-lg z-20 overflow-hidden">
                                    {seedResults.map(song => (
                                        <div 
                                            key={song.id} 
                                            onClick={() => handleSelectSeed(song)}
                                            className="flex items-center gap-3 p-2 hover:bg-[#3D0050] cursor-pointer transition-colors"
                                        >
                        ...
                        <div className="flex items-center justify-between bg-[#2A0038] p-2 rounded-lg">
                            ...
                            <button onClick={() => { setSelectedSeed(null); setSeedQuery(''); }} className="p-1 hover:bg-[#0A000F]/20 rounded-full">
