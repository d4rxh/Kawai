...
import { Play, Pause, SkipBack, SkipForward, ChevronDown, Download, Heart, Loader2, Shuffle, Repeat, Repeat1, CheckCircle2, PlusCircle, Sparkles, PanelRightClose, ListMusic } from 'lucide-react';
...
import { SpeedButton } from './SpeedModal';
import { AutoDJ } from './AutoDJ';
import { LyricsButton } from './LyricsScreen';
import { CarModeButton } from './CarMode';
        ...
        {isFullScreen ? (
            ...
            >
                ...
                >
                    ...
                    >
                        <div className="relative w-full aspect-square md:max-w-[340px] mx-auto rounded-full overflow-hidden bg-[#130018] cursor-pointer shadow-2xl border-4 border-[#FF6B9D]/20" style={{ animation: isPlaying ? "spin 18s linear infinite" : "none" }}>
                            <AnimatePresence mode="wait">
                                {showArtVideo ? (
                                    <motion.div 
                    ...
                    <div className="flex flex-col shrink-0 gap-4 md:gap-6">
                        ...
                        <div className="flex items-center justify-between px-2 mb-2" onClick={(e) => e.stopPropagation()}>
                          ...
                          <button
                            ...
                            className={`p-2 rounded-full transition-all ${queue.length > 0 ? 'text-[#FF6B9D]' : 'text-white/40 hover:text-white'}`}
                            title="Queue"
                          >
                            <ListMusic size={20} />
                          </button>
            ...
            <motion.div 
                ...
                transition={{ type: "spring", damping: 25, stiffness: 200 }}
                className={`fixed z-[200] overflow-hidden flex flex-col isolate cursor-pointer ${miniPlayerStyle === "island" ? "left-1/2 -translate-x-1/2 bottom-8 w-auto min-w-[240px] max-w-[340px] h-[52px] rounded-full px-4 bg-black border border-white/10" : "left-0 right-0 mx-auto w-[calc(100%-32px)] max-w-[400px] h-[64px] rounded-xl p-1.5 bg-[#130018]"} ${navPosition === "bottom" ? "bottom-[88px]" : "bottom-6"} ${miniPlayerStyle === "island" ? "" : ""}`}
                ...
                <div className="relative flex items-center h-full w-full gap-3 px-2">
                    ...
                    <div className="flex-1 min-w-0 pr-2 flex flex-col justify-center">
                        ...
                        <div className="flex items-center gap-1.5">
                          <MoodRing />
                          <span className="text-white/70 text-xs truncate">{currentSong.artists.primary[0]?.name}</span>
                        </div>
                    </div>
