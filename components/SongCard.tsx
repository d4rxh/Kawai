...
const SongCardBase: React.FC<SongCardProps> = ({ item, onPlay, subtitle, round = false }) => {
  ...
  return (
    <div
      onClick={handleClick}
      className="group relative bg-transparent cursor-pointer w-[140px] md:w-[160px] shrink-0 active:scale-[0.97] transition-transform duration-150"
      ...
      <div className={`relative w-full aspect-square mb-3 ${round ? 'rounded-full' : 'rounded-md'} overflow-hidden bg-[#130018]`}>
        ...
        {!round && item.type !== 'artist' && (
          <button
            ...
            className="absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 bg-white text-black rounded-full p-2.5 flex items-center justify-center z-20 active:scale-95"
          >
